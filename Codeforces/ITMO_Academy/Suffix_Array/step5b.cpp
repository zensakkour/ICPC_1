...
string longest_common_substring(string s, string t) {
    ...
    for (int i = 1; i < combined.size(); i++) {
        if ((p[i] < n && p[i-1] >= n) || (p[i] >= n && p[i-1] < n)) { 
            ...
            else if (lcp[i-1] == max_len) {
                if (combined.substr(p[i], max_len) < combined.substr(start_pos, max_len)) {
                    start_pos = p[i];
                }
            }
        }
    ...
    return combined.substr(start_pos, max_len);  
}
...
int main() {
    ...
    string result = longest_common_substring(s, t);
    cout << result << endl;
    return 0;
}
